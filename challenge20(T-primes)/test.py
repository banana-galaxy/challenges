...
# check for not allowed stuff
def scan(file):
    with open(file) as f:
        for i in f.readlines():
            if 'import' in i:
                return [False, 'libraries in use']
            elif 'input' in i:
                return [False, 'getting user input']
    return [True, '']
...
try:
    ...
    for index, f in enumerate(files):
        if f.endswith(".py"):
            scann = scan(os.path.join(os.path.dirname(os.path.realpath(__file__)), "solutions", f))
            if not scann[0]:
                wrong.append(f"\n{f.split('.')[0]}\nnot allowed: {scann[1]}")
                continue
