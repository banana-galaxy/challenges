        ...
        if f.endswith(".py"):
            ...
            for i, x in enumerate(checks):
                ...
                for _ in slow:
                    if _ in f:
                        print(
                            f"solution: {index + 1}/{len(files)} testcase: {i + 1}/{len(checks)//20}  passed:{len(winners)} failed:{len(wrong)} extremely slow solution, running 5% of testcases only",
                            " " * len(str(len(checks))), end="\r")
                        if i+1>=len(checks)//20:
                            break_check = True
                    else:
                ...
                try:
                    disablePrint()
                    output = unit.solution(*case)
                    enablePrint()
                    if output != outs[i]:
                        passed = False
                        wrong.append(f"\n{f.split('.')[0]}\ntest:{x}\nsolution output:{repr(output)}\ncorrect output:{outs[i]}")
                        break
                except BaseException as e:
                    enablePrint()
                    passed = False
...
if usr.lower() != "n":
    ...
    for i in winners:
        update_board.won(file, i[1], 19)
    for i in wrong:
        i = i.split("\n")[1]
        update_board.lost(file, i, 19)
